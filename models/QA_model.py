...
import transformers
from transformers import CLIPVisionConfig#, MixtralForCausalLM
...
class QA_model(nn.Module):
    ...
    def generate_long_sentence(self,input_ids,images):
        with torch.no_grad():
            ...
            input_embedding = self.llamacasual.get_input_embeddings()(input_ids)
            input_embedding = torch.cat([features,input_embedding], dim=1).half()

            #generation = self.llamacasual(inputs_embeds = input_embedding)['logits']
            generation = self.llamacasual.generate(inputs_embeds = input_embedding, max_new_tokens =50)
            return generation

# class MoeQA_model(QA_model):
#     def __init__(self, model_args):  
#         super(MoeQA_model,self).__init__(model_args)  
#         # self.llamacasual.__del__()
#         model = MixtralForCausalLM.from_pretrained(model_args.model_path).half().to('cuda')
#         self.llamacasual = model